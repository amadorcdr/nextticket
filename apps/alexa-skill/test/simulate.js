 ...
 * ============================================================================
 * Levanta una API falsa que responde con la forma EXACTA del backend real
 * (paginado { data, meta }, camelCase, publicName / eventPrice /
 * availableCapacity, byEventZone con ticketsSold) y corre la skill contra ella.
 *
...
const SEEDS = {
  jaguarmorado: { id: "usr-100", name: "Carlos Rivera", email: "organizador@test.com", role: { name: "ORGANIZER" } },
  faroazul: { id: "usr-200", name: "Admin NextTicket", email: "admin@test.com", role: { name: "ADMIN" } },
  roblesereno: { id: "usr-300", name: "Juan Pérez", email: "cliente@test.com", role: { name: "CLIENT" } },
};

// Mismo shape que PurchasesStatsResponseDto: byEventZone con revenue y ticketsSold.
const STATS = {
  "evt-100": {
    totalRevenue: 5000,
    recentPurchasesCount: 1,
    byEventZone: [{ eventZoneId: "zon-101", revenue: 5000, ticketsSold: 2 }],
  },
  "evt-200": {
    totalRevenue: 1200,
    recentPurchasesCount: 1,
    byEventZone: [{ eventZoneId: "zon-201", revenue: 1200, ticketsSold: 1 }],
  },
};
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && path === "/auth/alexa/seed") {
    ...
    req.on("end", () => {
      ...
      if (!user) return json(res, 401, { message: "Palabra clave no válida" });
  ...
  if (path === "/purchases/stats") {
    ...
    if (eventId) {
      const stats = STATS[eventId] || { totalRevenue: 0, recentPurchasesCount: 0, byEventZone: [] };
      return json(res, 200, Object.assign({ from: null, to: null }, stats));
    }
...
async function main() {
  ...
  await turn("Login como cliente", intentRequest("CaptureInputIntent", { userInput: slot("userInput", "roble sereno") }));
  ...
  console.log("\n════════ ESCENARIO 4: PALABRA CLAVE INCORRECTA ════════");
  attributes = {};
  await turn("Abrir la skill", launchRequest());
