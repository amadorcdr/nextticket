 ...
 *  10. SKILL BUILDER
 *
 * VARIABLES DE ENTORNO (Alexa Console -> Code -> no; se ponen en la Lambda de
 * AWS, o en "Alexa-Hosted" desde la pestaña Code > Environment Variables):
 *
 *   API_BASE_URL                    https://xxxx.ngrok-free.app   (SIN / final)
 *   API_TIMEOUT_MS                  4000   (Alexa corta a los 8 s)
 *   DYNAMODB_PERSISTENCE_TABLE_NAME (la crea Alexa-Hosted sola)
...
const repo = {
  ...
  /** Ventas de un evento: la recaudación viene del backend, lo vendido se deriva. */
  ...
  /**
   * Recaudación de un mes. Requiere el filtro from/to en GET /purchases/stats
   * (ver backend-patch/README.md). Si el backend aún no lo soporta, ignora los
   * parámetros y devuelve el total global: se avisa en la respuesta.
   */
  async getRevenueByMonth(token, monthNumber, year) {
    ...
    return {
      ...
      filtered: Boolean(stats.from || stats.periodFrom),
    };
  },
};
...
const GetTotalRevenueByPeriodIntentHandler = {
  ...
  handle: withAuth("GetTotalRevenueByPeriod", [ROLES.ADMIN], async (handlerInput, session) => {
    ...
    let speech;
    if (!result.filtered) {
      // El backend todavía no filtra por fecha: se dice la verdad al usuario.
