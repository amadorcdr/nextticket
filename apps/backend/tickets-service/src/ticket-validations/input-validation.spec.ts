...
describe('CreateValidationDto', () => {
  ...
  it('accepts a 64 char SHA-256 hexadecimal hash', async () => {
    await expect(pipe.transform({ qrHash: QR_HASH }, meta)).resolves.toEqual(
      expect.objectContaining({ qrHash: QR_HASH }),
    );
  });

  it('rejects a hash with the wrong length', async () => {
    await expect(pipe.transform({ qrHash: 'abc123' }, meta)).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
