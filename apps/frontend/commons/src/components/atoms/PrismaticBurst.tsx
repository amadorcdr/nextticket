...
float edgeFade(vec2 frag, vec2 res, vec2 offset, float t){
    ...
    float n = fastHash(frag * 0.15 + t * 10.0);
    float dn = (n - 0.5) * 0.001 * s;
...
void main(){
    ...
    float n = fastHash(frag + uTime * 5.0);
    
    vec4 c = cos(t * 0.2 + vec4(0.0, 33.0, 11.0, 0.0));
    float amp = clamp(uDistort, 0.0, 50.0) * 0.15;

    mat3 transformMat = mat3(1.0);
    if(uAnimType == 0){
        transformMat = mat3(
            c.x, 0.0, c.z,
            0.0, 1.0, 0.0,
        ...
        transformMat = rotZ(ang.z) * rotY(ang.y) * rotX(ang.x);
    } else if(uAnimType == 2){
        vec2 m = uMouse * 2.0 - 1.0;
        vec3 ang = vec3(m.y * 0.6, m.x * 0.6, 0.0);
        transformMat = rotY(ang.y) * rotX(ang.x);
    }

    bool useColors = uColorCount > 0;
    bool useRays = uRayCount > 0;
    float fRayCount = float(uRayCount);
    ...
    for (int i = 0; i < 44; ++i) {
        ...
        float ampGrow = amp * grow; 

        float a1 = ampGrow * 0.8 * sin(Pl.x * 0.55 + t * 0.6)
                 + ampGrow * 0.7 * sin(Pl.y * 0.50 - t * 0.5)
                 + ampGrow * 0.6 * sin(Pl.z * 0.60 + t * 0.7);
        float a2 = 0.5 * ampGrow * 0.8 * sin(Pl.z * 0.55 + t * 0.6 + 3.1)
                 + 0.5 * ampGrow * 0.7 * sin(Pl.y * 0.50 - t * 0.5 * 0.9 + 3.1)
                 + 0.5 * ampGrow * 0.6 * sin(Pl.x * 0.60 + t * 0.7 * 0.9 + 3.1);

        float s1 = sin(a1), c1 = cos(a1);
        Pl.xz = mat2(c1, -s1, s1, c1) * Pl.xz;

        float s2 = sin(a2), c2 = cos(a2);
        Pl.xy = mat2(c2, -s2, s2, c2) * Pl.xy;
        vec3 Pb = Pl;
        ...
        if (useRays) {
            ...
            float comb = 0.5 + 0.5 * cos(fRayCount * ang);
            comb = comb * comb * comb;
    ...
    col *= edgeFade(frag, uResolution, uOffset, uTime);
    col *= uIntensity;
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    // Eliminamos los estilos inline de JS y dejamos que Tailwind lo maneje en el div padre
  ...
  // Aplicamos TODA la magia oscura (y clara) de Tailwind directamente en el div contenedor
  return (
    <div
      className="
        w-full h-full relative overflow-hidden rounded-[10px]
        mix-blend-multiply invert hue-rotate-180 
        dark:mix-blend-lighten dark:invert-0 dark:hue-rotate-0
      "
      ref={containerRef}
    />
