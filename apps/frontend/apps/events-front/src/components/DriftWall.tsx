...
export const DriftWall = ({
  ...
  const columnMeta = useMemo<ColumnMeta[]>(() => {
    ...
    return columnItems.map(col => {
      ...
      const copies = Math.max(2, Math.ceil((containerHeight * 2.5) / copyHeight) + 1);
  ...
  const innerClass = cx(
    'pointer-events-none absolute inset-[calc(var(--dw-gap)/2)] block overflow-hidden bg-background',
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className={innerClass}>
        ...
        {item.title && (
          <span className={cx(
            'absolute inset-0 flex flex-col gap-2 items-center justify-center p-4 text-center z-10',
            'bg-black/40 opacity-0 transition-opacity duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)]',
            'group-[.is-active]/tile:opacity-100 group-focus-visible/tile:opacity-100'
          )}>
            <h4 className="line-clamp-2 text-white">{item.title}</h4>
            {item.href && (
              <Router.Link
                to={item.href}
                className="button button--md button--primary pointer-events-auto"
              >
                Ver detalles
              </Router.Link>
            )}
